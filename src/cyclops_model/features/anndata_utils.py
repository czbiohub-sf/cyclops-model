...
def load_features_corrected(
    ...
    *,
    ...
    ``cyclops_model.data.iss_drift_fix`` produces a sibling parquet
    ...
    if not sidecar.exists():
        warnings.warn(
            ...
            "`python -m cyclops_model.data.iss_drift_fix --exp <exp>` to build it."
        )
        return adata
    ...
    if len(patch) != adata.n_obs:
        raise ValueError(
            ...
            "`python -m cyclops_model.data.iss_drift_fix --exp <exp> --overwrite`."
        )
...
def normalize_guide_adata(
    ...
    Uses :func:`cyclops_utils.analysis.normalization.zscore_normalize` under the hood.
    ...
    """
    from cyclops_utils.analysis.normalization import zscore_normalize

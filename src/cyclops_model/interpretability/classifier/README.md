...
`cyclops_model` is required at run time.

## Install

Needs the optional `classifier` extra (`hydra-core`, `omegaconf`, `pyarrow`, `tqdm`,
`matplotlib`) — declared in `cyclops_model/pyproject.toml` and *not* part of the default
monorepo env:

```bash
uv pip install -e "cyclops_model[classifier]"
```
...
directory* (`src/cyclops_model/interpretability/classifier/configs/ebi_complexes.csv`),
i.e. they assume you run from the `cyclops_model/` repo root. From anywhere else,
...
```bash
uv run python -m cyclops_model.interpretability.classifier.score \

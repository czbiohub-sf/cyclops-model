...
- **C** — MLP on **CellDINO** embeddings of the same crops. Option C runs the
  local CellDINO encoder (`cyclops_model.models.cell_dino`) over the identical crops
...
uv run python -m cyclops_model.interpretability.diffae.classifier.run --model B --gene HSPA5
...
uv run python -m cyclops_model.interpretability.diffae.classifier.submit --gene HSPA5

# sweep: all EBI complexes + NTC control, model C
uv run python -m cyclops_model.interpretability.diffae.classifier.submit --grain complex --all-classes --models C

# then rank them
uv run python -m cyclops_model.interpretability.diffae.classifier.aggregate --grain complex --model C
```

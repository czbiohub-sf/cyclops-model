...
python -m cyclops_model.interpretability.diffae.classifier.submit --grain complex --all-classes --models C
python -m cyclops_model.interpretability.diffae.classifier.aggregate --grain complex --model C

# Stage 2 — train the conditional DiffAE (resume-able; gate = embedding/noise ratio)
python -m cyclops_model.interpretability.diffae.generator.submit --epochs 120 --batch-size 48
python -m cyclops_model.interpretability.diffae.generator.diagnose_conditioning   # conditioning-strength check

# Stage 3 — directions + counterfactual traversal for a target
python -m cyclops_model.interpretability.diffae.directions.submit --grain geneKO --target HSPA5
```

...
```bash
python -m cyclops_model.post_process.combination.pca_optimization [FLAGS]
```

Full flag reference (60+ options):

```bash
python -m cyclops_model.post_process.combination.pca_optimization --help
```
...
```bash
python -m cyclops_model.post_process.combination.pca_optimization --config my_run.yml
```
...
```python
from cyclops_model.post_process.combination.pca_optimization import run_from_config
